...
class HPresolve {
 ...
 public:
  ...
  HPresolve::Result AggregationofVI(HighsMipSolver& mipsolver,
                                HighsPostsolveStack& postsolve_stack);
  HPresolve::Result processLinearImpliedBounds(HighsMipSolver& mipsolver, HighsPostsolveStack& postsolve_stack);

 private:
  // Types and helpers shared by the variable-bound aggregation and the
  // implied-bound tightening rules.
  enum BoundType { kLowerBound, kUpperBound };
  struct BoundConstraint {
    HighsInt mainVar;        // main (non-binary or more frequent binary) variable
    HighsInt binaryVar;      // associated binary variable
    double coefficient;      // coefficient of the binary variable
    BoundType type;          // lower or upper bound constraint
    bool complement = false; // whether the binary variable is complemented
    bool reversed = false;   // whether the original row was reversed
  };

  // Returns true if the size-two row consists of either one non-binary
  // variable with coefficient +/-1 and one binary variable, or of two binary
  // variables. In the latter case the variable occurring more often in other
  // size-two rows is chosen as the main variable.
  bool parseVariableBoundRow(HighsInt row, const HighsMipSolver& mipsolver,
                             std::unordered_map<HighsInt, int>& freq,
                             BoundConstraint& bc, bool& inverse);

  // Frequency of binary variables in undeleted size-two rows.
  std::unordered_map<HighsInt, int>
  computeBinaryVariableFrequency(const HighsMipSolver& mipsolver) const;

  bool isBinary(const HighsMipSolver& mipsolver, HighsInt col) const;

  // Tighten the lower/upper bound of col implied by y >= bound + coef*x /
  // y <= bound + coef*x with x in [0,1].
  void tightenColLower(HighsInt col, double bound, double coef);
  void tightenColUpper(HighsInt col, double bound, double coef);
};
