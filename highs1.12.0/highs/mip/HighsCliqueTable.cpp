...
void HighsCliqueTable::cliqueCover(std::vector<CliqueVar>& clqVars,
  ...
  std::vector<bool> covered(clqVars.size(), false);
  std::vector<HighsInt> neighbourhoodInds;
  neighbourhoodInds.reserve(clqVars.size());

  HighsInt numClqVars = clqVars.size();

  HighsInt currentIndex = 0;

  // Order the variables by decreasing number of cliques.
  std::sort(clqVars.begin(), clqVars.end(),
            [this](const CliqueVar& a, const CliqueVar& b) {
              return this->numCliques(a.col, a.val) >
                     this->numCliques(b.col, b.val);
            });

  while (currentIndex < numClqVars) {
    if (covered[currentIndex]) {
      currentIndex++;
      continue;
    }

    // Initialize the current clique with the first uncovered variable.
    std::vector<CliqueVar> currentClique;
    currentClique.push_back(clqVars[currentIndex]);
    covered[currentIndex] = true;

    HighsInt extensionStart = currentIndex + 1;
    HighsInt extensionEnd = numClqVars;

    // Restrict the candidates to the neighbourhood of the seed variable.
    extensionEnd = partitionNeighbourhood(
        neighbourhoodInds, numNeighbourhoodQueries, clqVars[currentIndex],
        clqVars.data() + extensionStart,
        extensionEnd - extensionStart) +
        extensionStart;

    // Extend the clique.
    while (extensionStart < extensionEnd) {
      // Sort the candidates by decreasing number of cliques.
      std::sort(clqVars.begin() + extensionStart,
      ...
      // Take the first candidate and restrict the remaining ones to its
      // neighbourhood.
      currentClique.push_back(clqVars[extensionStart]);
      covered[extensionStart] = true;

      HighsInt chosenPos = extensionStart;
      extensionStart++;

      extensionEnd = partitionNeighbourhood(
          neighbourhoodInds, numNeighbourhoodQueries, clqVars[chosenPos],
          clqVars.data() + extensionStart,
          extensionEnd - extensionStart) +
          extensionStart;
    }

    // Add all already covered variables that conflict with every member of
    // the current clique.
    for (HighsInt i = 0; i <= currentIndex; ++i) {
      const auto& candidate = clqVars[i];
      bool conflictWithAll = true;

      for (const auto& varInClique : currentClique) {
        if (!this->haveCommonClique(varInClique, candidate)) {
          conflictWithAll = false;
          break;
        }
      }

      if (conflictWithAll) currentClique.push_back(candidate);
    }

    cover.push_back(std::move(currentClique));

    currentIndex++;
    while (currentIndex < numClqVars && covered[currentIndex]) {
      currentIndex++;
    }
  }

  // Count how often each variable occurs in the cover.
  std::vector<HighsInt> count(clqVars.size(), 0);
  for (const auto& clique : cover) {
    for (const auto& var : clique) {
      HighsInt varIdx = -1;
      for (size_t i = 0; i < clqVars.size(); ++i) {
        if (clqVars[i].col == var.col && clqVars[i].val == var.val) {
          varIdx = i;
          break;
        }
      }
      if (varIdx != -1) count[varIdx]++;
    }
  }

  // Mark cliques as redundant if all of their variables also occur in other
  // cliques.
  std::vector<bool> redundant(cover.size(), false);
  for (size_t i = 0; i < cover.size(); ++i) {
    if (redundant[i]) continue;

    bool canRemove = true;
    for (const auto& var : cover[i]) {
      HighsInt varIdx = -1;
      for (size_t j = 0; j < clqVars.size(); ++j) {
        if (clqVars[j].col == var.col && clqVars[j].val == var.val) {
          varIdx = j;
          break;
        }
      }
      // A variable occurring only once cannot be removed.
      if (varIdx == -1 || count[varIdx] < 2) {
        canRemove = false;
        break;
      }
    }

    // Remove redundant cliques and update the occurrence counts.
    if (canRemove) {
      redundant[i] = true;
      for (const auto& var : cover[i]) {
        HighsInt varIdx = -1;
  ...
  // Drop the redundant cliques.
  std::vector<std::vector<CliqueVar>> tempCover;
  for (size_t i = 0; i < cover.size(); ++i) {
    if (!redundant[i]) tempCover.emplace_back(std::move(cover[i]));
  }
  cover.swap(tempCover);
}

HighsCliqueTable::CliqueRange
HighsCliqueTable::cliqueRange(const HighsInt cliqueId) const {
  const HighsInt start = cliques[cliqueId].start;
  const HighsInt end   = cliques[cliqueId].end;
  return { cliqueentries.data() + start,
           cliqueentries.data() + end   };
}

void CliqueAdjacency::build(const HighsCliqueTable& cliquetable) {
  adj.clear();

  // Cliques larger than this are ignored for the adjacency.
  constexpr HighsInt MAX_CLIQUE_SIZE = 200;
  HighsInt numCliques = cliquetable.numCliques();

  // Map each node to the cliques it belongs to.
  std::unordered_map<Node, std::vector<HighsInt>, NodeHash> node_to_cliques;
  node_to_cliques.reserve(numCliques * 2);

  for (HighsInt cid = 0; cid < numCliques; ++cid) {
    auto members = cliquetable.cliqueRange(cid);
    HighsInt clique_size = members.endPtr() - members.beginPtr();

    if (clique_size > MAX_CLIQUE_SIZE) continue;

    for (const auto* p = members.beginPtr(); p != members.endPtr(); ++p) {
      Node u = *p;
      node_to_cliques[u].push_back(cid);
    }
  }

  // Build the adjacency lists.
  for (const auto& kv : node_to_cliques) {
    const Node& u = kv.first;
    const auto& cliques = kv.second;
    auto& neighbors = adj[key(u)];

    for (HighsInt cid : cliques) {
      auto members = cliquetable.cliqueRange(cid);
      for (const auto* p = members.beginPtr(); p != members.endPtr(); ++p) {
        Node v = *p;
        if (v.col == u.col && v.val == u.val) continue;  // skip self
        neighbors.push_back(v);
      }
    }

    // Deduplicate the neighbors.
...
const CliqueAdjacency::AdjList& CliqueAdjacency::neighbors(const Node& node) const {
  static const AdjList empty;
  auto it = adj.find(key(node));
  if (it == adj.end()) return empty;
  return it->second;
}

void HighsCliqueTable::newbuildFrom(const HighsLp* origModel,
                                    const HighsCliqueTable& init) {
  assert(init.colsubstituted.size() == colsubstituted.size());
  HighsInt ncols = init.colsubstituted.size();

  HighsCliqueTable newCliqueTable(ncols);
  // Keep the presolve flags of the current table (as in buildFrom).
  newCliqueTable.setPresolveFlag(inPresolve);
  newCliqueTable.setPresolveFlag(minEntriesForParallelism);
  ...
  for (HighsInt i = 0; i != ncliques; ++i) {
    const auto& c = init.cliques[i];
    if (c.start == -1) continue;

    HighsInt numvars = c.end - c.start;
    if (numvars - c.numZeroFixed <= 1) continue;

    HighsInt s = c.start;
    HighsInt e = c.end;

    clqBuffer.assign(init.cliqueentries.begin() + s,
                     init.cliqueentries.begin() + e);

    // Only keep binary variables.
    clqBuffer.erase(std::remove_if(clqBuffer.begin(), clqBuffer.end(),
                                   [origModel](const CliqueVar& v) {
    ...
    newCliqueTable.doAddClique(clqBuffer.data(), (HighsInt)clqBuffer.size(),
                               false, origin);
  }
  ...
  // Move the members explicitly instead of relying on the implicit move
  // assignment.
  this->cliques = std::move(newCliqueTable.cliques);
  this->cliqueentries = std::move(newCliqueTable.cliqueentries);
  this->colsubstituted = std::move(newCliqueTable.colsubstituted);
  this->substitutions = std::move(newCliqueTable.substitutions);
  this->inPresolve = newCliqueTable.inPresolve;
  this->minEntriesForParallelism = newCliqueTable.minEntriesForParallelism;

  // Sanity check the current object.
  for (size_t ii = 0; ii < this->cliques.size(); ++ii) {
    const auto& c2 = this->cliques[ii];
    if (c2.start == -1) continue;
    assert(c2.start >= 0 && c2.end > c2.start &&
           c2.end <= (HighsInt)this->cliqueentries.size());
  }
}