...
class HighsCliqueTable {
 public:
  ...
  /** Access the member range [begin, end) of a clique. */
  struct CliqueRange {
    const CliqueVar* begin;
    const CliqueVar* end;
    const CliqueVar* beginPtr() const { return begin; }
    const CliqueVar* endPtr()   const { return end;   }
  };
  /** Return the member range of a clique in O(1). */
  CliqueRange cliqueRange(HighsInt cliqueId) const;

  /** Call fn(cliqueId) for every clique containing the variable v. */
  template <class Fn>
  void forEachCliqueOf(const CliqueVar& v, Fn&& fn) const {
    invertedHashListSizeTwo[v.index()].for_each([&fn](HighsInt cliqueid) {
      fn(cliqueid);  
    });
    invertedHashList[v.index()].for_each([&fn](HighsInt cliqueid) {
      fn(cliqueid);  
    });
  }
 ...
 public:
  ...
  void newbuildFrom(const HighsLp* origModel, const HighsCliqueTable& init);
...
class CliqueAdjacency {
 public:
  using Node = HighsCliqueTable::CliqueVar;
  using AdjList = std::vector<Node>;

  struct NodeHash {
    std::size_t operator()(const Node& n) const noexcept {
      // Combine col and val into a single hash.
      return std::hash<HighsInt>()(n.col) ^ (std::hash<int>()(n.val) << 1);
    }
  };

  // Build the adjacency lists from the clique table.
  void build(const HighsCliqueTable& cliquetable);

  // Neighbors of the given node.
  const AdjList& neighbors(const Node& node) const;

 private:
  // Map (col, val) -> adjacency list.
  std::unordered_map<long long, AdjList> adj;

  // Encode (col, val) as a unique key.
  inline long long key(const Node& n) const {
    return (static_cast<long long>(n.col) << 1) | (n.val ? 1 : 0);
  }
};
